...
\subsection{Routing Control Plataform}
...
El Controlador expone un conjunto de operaciones a las aplicaciones de la capa superior(capa de Aplicación) para la manipulación de los dispositivos de la capa inferior (capa de infraestructura). Esta API de operaciones es conocida en el modelo bajo el nombre de Interfaz Norte.\\

Finalmente, la capa de aplicaciones contiene implementado en software (aplicaciones SDN) toda la inteligencia que originalmente se agregaba en un dispositivo de la capa de infraestructura, por ejemplo la implementaci\'on de protocolos de red. 
...
\item \textbf{Despliegue de Servicios:}
...
En Uruguay ANTEL (el principal proveedor de servicios de telecomunicaciones del país) ofrece servicios de redes privadas basados en tres tecnolog\'ias principalmente\footnote{Informaci\'on extraida en base a los servicios comerciales ofrecidos por ANTEL en su sitio oficial \cite{ANTELVPN}}: Lan to Lan con conexiones Ethernet punto a punto, VLAN Hub tmbi\'en con conexiones Ethernet y VPN IP/MPLS multipunto con QoS opcionalmente.\\
...
Multiprotocol Label Switching (MPLS), es un mecanismo de transporte de datos desarrollado y estandarizado por el IETF, en particular por los RFC 3031\cite{rosen2001multiprotocol}, 3032\citep{rosen2001mpls} y 3814\citep{nadeau2004multiprotocol}.\\
...
Actualmente cuenta con una gran adopción por parte de fabricantes y poco a poco se ha convertido en el mecanismo predilecto para la construcci\'on de servicios de redes privadas, sustituyendo poco a poco los servicios de Frame Relay y ATM.\\
...
Alguno de los principales términos en la definici\'on del protocolo MPLS son:
...
NetFPGA\cite{NetFPGA1} es una plataforma de hardware reconfigurable y software de c\'odigo abierto, flexible y potente, diseñada para un uso principalmente académico en tareas de investigación y enseñanza.\\
