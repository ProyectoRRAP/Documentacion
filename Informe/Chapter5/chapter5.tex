...
A modo de introducci\'on RAUFlow es una aplicaci\'on de gesti\'on de red, basada en el enfoque SDN e implementada utilizando entre otras tecnolog\'ias el software de control Ryu.

Es m\'as que una aplicaci\'on del plano de control OpenFlow, puesto que en su arquitectura se incluyen componentes que no son implementadas por un controlador OpenFlow.\\ 

La arquitectura de RAUFlow (ver figura \ref{fig:VistaComponentes2}), se caracteriza por un diseño en capas l\'ogicas. En particular esta compuesta de cuatro capas: (1) capa de presentación, (2) capa de aplicaciones Ryu, (3) capa de negocios, y (4) capa de dispositivos.\\
...
A continuaci\'on se explican en detalle cada una de estas capas.
...
Ryu incluye tres aplicaciones dise\~nadas para implementar una interfaz gr\'afica, consistente en un p\'agina web implementada sobre html, javascript y web sockets, que permite visualizar de forma gr\'aica la red en su totalidad. Estos son los dispositivos del datapath con sus interfaces y los enlaces existentes.

Por esta razón, orientado a proveer de una representaci\'on gr\'afica del estado de la red en el prototipo (el estado de la topolog\'ia y sus dispositivos), se incluyen en el dise\~no estas aplicaciones.
