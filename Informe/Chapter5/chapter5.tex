...
Conceptualmente RAUFlow es una aplicaci\'on de gestión de red, basada en el enfoque de SDN e implementada utilizando el software de Control Ryu y el protocolo OpenFlow entre otras tecnologías. No obstante, entre las otras tecnologías mencionadas con las cuales interactua la aplicaci\'on se encuentran la suite de ruteo Quagga, de la cual se obtiene información de la base de datos topol\'ogica de dicha herramienta y el protocolo SNMP. 

Esto conlleva al desarrollo de un conjunto de modulos especiales para la interacci\'on entre estas componentes y las aplicaciones Ryu por ejemplo. 

Analizando globalmente a la aplicaci\'on, su arquitectura (ver figura \ref{fig:VistaComponentes2}), se caracteriza por un diseño en tres capas l\'ogicas: (1) capa de presentaci\'on, (2) capa de control y (3) capa de dispositivos.\\
...
En las siguientes secciones se explican en detalle cada una de estas capas, comenzando por la capa de control.

\subsection{Capa de Control}

La capa de control esta conformada por los diferentes procesos y aplicaciones encargados de implementar, valga la redundancia el plano de control del prototipo; de ahí el nombre capa de control.

Estos procesos son: El Controlador Ryu sobre el cual se ejecuta un conjunto de aplicaciones OpenFlow, un m\'odulo denominado LSDBSync encargado de sincronizar la informaci\'on de la Link-State-Database entre la instancia de Quagga que se ejecuta en el controlador y las aplicaciones de Ryu, y finalmente  otro modulo denominado SNMP Administrador encargado de la comunicaci\'on con los agentes SNMP instalados en cada nodo del prototipo. 

A continuaci\'on se explican en detalle cada una de estas componentes.
