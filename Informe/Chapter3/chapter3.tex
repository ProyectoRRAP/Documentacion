...
\section[Implementaci\'on de VPN IP/MPLS]{Implementaci\'on de VPN IP/MPLS}
...
Sin embargo varios aspectos de la arquitectura deben ser definidos para contar con una implementaci\'on de este tipo. Debe definirse la estrategia para calcular las rutas o caminos en la red del prototipo(algoritmo de ruteo), la forma en que se asignan y distribuyen etiquetas MPLS para implementar el plano de reenvío(algoritmo de distribución de etiquetas), como se implementa la clasificaci\'on de tr\'afico, y de que forma se implementan múltiples caminos para un mismo destino, ya sea para priorizar tr\'afico por tipo de aplicaci\'on o para hacer balanceo de carga. 

A continuaci\'on se explican las decisiones tomadas para cada uno de los aspectos anteriores.

\subsection{Algoritmo de Ruteo}
El calculo de rutas puede realizarse en base al protocolo IP, sacando provecho de los diferentes algoritmos y protocolos de ruteo existentes en base a IP, mucho de los cuales ya tienen varios años de existencia, y probaron ampliamente su correctitud y robustez.\\

Dado el enfoque centralizado del plano de control de SDN, se puede utilizar tanto un algoritmo de ruteo centralizado como distribu\'ido, existiendo en cada enfoque ventajas y desventajas.

Por un lado parece atractivo combinar los enfoques centralizado de SDN con el enfoque de un algoritmo de ruteo distribuido, con el objetivo de reducir la sobrecarga de procesamiento en el controlador. Siguiendo con este enfoque OSPF se presenta como una alternativa robusta, probada y confiable, adem\'as de que se cuentan con buenas implementaciones de software libre de este protocolo (por ejemplo la suite Quagga).

Sin embargo uno de los requerimientos fuertes sobre el prototipo, es la facilidad para hacer balanceo de carga y construcci\'on de caminos alternativos para un mismo origen y destino en funci\'on de clasificaciones de tr\'afico. En primer lugar, para implementar esto es necesario que el algoritmo de ruteo utilizado permita la incorporaci\'on de restricciones adem\'as de la capacidad para calcular m\'as de un camino posible. En segundo lugar muchas de estas restricciones son din\'amicas, puesto que están asociadas a servicios de VPN que se dan de alta en el prototipo y pol\'iticas de ingenier\'ia de tr\'afico como priorizaci\'on de tr\'afico, alta de caminos alternativos por aplicaci\'on, balanceo de carga, etc.\\

Si bien OSPF puede ser extendido como protocolo para contemplar restricciones, y de hecho existe y se llama CSPF(Constrained Shortest Path First), no todas las funcionalidades asociadas al computo de rutas, pueden ser traducidas a restricciones en este algor\'itmo. 

Por otro lado si parece razonable contemplarlas en un algoritmo de ruteo centralizado, a ejecutarse sobre el controlador SDN, el cual por practicidad adem\'as puede estar inspirado en CSPF pero con un enfoque centralizado.\\
