...
Como se menciona en el capitulo 2, en la arquitectura OpenFlow/SDN se tiene dos entidades fundamentales: el Controlador y el switch compatible con OpenFlow. En la arquitectura del prototipo se  parte de esta idea y se le incorporan algunos conceptos adicionales como por ejemplo la herramienta Quagga.\\
...
Tal como se aprecia en la imagen ~\ref{fig:OpenSourceRArch0} el prototipo esta compuesto por nodos compatibles con el protocolo OpenFlow para la comunicaci\'on con el plano de control, y capaces de procesar tr\'afico mediante los protocolos IP y MPLS.

Cada nodo del prototipo esta implementado con lo que en este trabajo se denomina RAU-Switch, un switch compatible con OpenFlow capaz de construir una red de datos IP/MPLS, utilizando la tabla de flujos de OpenFlow para el procesamiento y reenv\'io de paquetes. En particular en la siguiente secci\'on se explica en detalle la arquitectura de este dispositivo.\\

Por otro lado se tiene la entidad de control sobre la cual se ejecuta la aplicaci\'on de SDN RAUFlow. Esta aplicaci\'on implementa los servicios de redes privadas virtuales, los algoritmos de ruteo y distribución de etiquetas e implementa una interfaz gr\'afica para la gesti\'on de la red, entre otras funcionalidades.\\ 

A continuaci\'on se explica en detalle cada componente.
...
Siguiendo la especificaci\'on de OpenFlow, Open vSwitch implementa el concepto de tabla de flujos. Estas tablas(OpenFlow implementa 256 tablas) son utilizadas por la aplicaci\'on RAUFlow para construir caminos en la topolog\'ia, instalando en forma de flujos OpenFlow las reglas de reenvío que sean necesarias en cada nodo del camino.

Para implementar el plano de reenvío en base a la conmutaci\'on de etiquetas MPLS, se utilizan primitivas del protocolo OpenFlow, que permiten colocar y extraer etiquetas MPLS de un paquete(PUSH y POP), así como clasificar un paquete acorde al valor de etiqueta MPLS contenida en \'el. 
...
Por su complejidad, m\'as adelante se destina un cap\'itulo entero a la comprensi\'on de las diferentes características en la implementaci\'on de dicha aplicaci\'on. 
