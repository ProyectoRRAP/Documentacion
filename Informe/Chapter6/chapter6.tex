...
De esta forma se implementa en el prototipo una VPN de capa 3 para la organizaci\'on mencionada, uniendo las tres sucursales de esta. A continuaci\'on se detallan las pruebas realizadas para cada uno de los aspectos mencionados anteriormente.
...
Tomando como ejemplo el caso del servicio S5, el camino te\'orico puede denotarse de la siguiente forma:
 
$$<(Poisson, nf_1)>$$

En otras palabras todo tr\'afico IP con origen en la subred B y destino a la subred C, es encaminado a través de la interfaz $nf_1$ en el nodo de ingreso \textit{Poisson}. Luego en el nodo de egreso \textit{Oz} es reenviado por la correspondiente interfaz del servicio.\\

Analizando las tablas de flujos de los nodos \textit{Poisson} y \textit{Oz}, puede comprobarse fácilmente la correspondencia entre el camino calculado y el camino te\'orico.

Por un lado en la tabla de flujos de \textit{Poisson} se tiene el siguiente flujo:
...
En pocas palabras este flujo toma todo paquete recibido por el puerto openflow identificado con el n\'umero 1(interfaz eth1), numeración IP origen 20.20.20.64/26(subred B) y destino 20.20.20.0/26 (subred C), decrementa el ttl del paquete, coloca un cabezal mpls con la etiqueta 34 y lo reenvía por el puerto openflow identificado con el n\'umero 3(interfaz nf1). Observar el valor de la etiqueta mpls colocado en el mismo para identificar el servicio(etiqueta interna). 
...
Por otro lado, una vez que los paquetes asociados al servicio S5 llegan a nodo \textit{Oz} ingresando por la interfaz nf1(puerto openflow n\'umero 3), debe ser retirada la etiqueta mpls interna y reenviarse a través de la interfaz eth1(puerto openflow n\'umero 1).

Este comportamiento es implementado en la tabla de flujos de Oz mediante el siguiente flujo:
...
De esta forma todos los paquetes asociados al servicio S5 son reenviados desde el nodo de ingreso Poisson al nodo de egreso Oz mediante un solo enlace utilizando un solo nivel de etiquetas mpls.\\

Analizando el camino que deben seguir los paquetes que atraviesan la red del laboratorio en el sentido inverso; es decir, desde la subred C hacia la subred B, se analiza el camino calculado para el tr\'afico asociado al servicio S6. 

El camino te\'orico para el tr\'afico asociado a dicho servicio es:

$$<(Oz, nf_0), (Galois, nf_2)>$$ 

Este camino es implementado mediante los siguientes flujos en la tabla del nodo \textit{Oz} por un lado:

\begin{center}
\textit{cookie=0.0, duration=1978.558s, table=0, n\_packets=0, n\_bytes=0, priority=5, \\
ip,in\_port=1, nw\_src=20.20.20.0/26,nw\_dst=20.20.20.64/26 \\
actions=dec\_ttl,push\_mpls:0x8847,set\_field:35->mpls\_label,set\_mpls\_ttl(128), goto\_table:1 \\
cookie=0.0, duration=1918.304s, table=0, n\_packets=0, n\_bytes=0, priority=5, \\
mpls,in\_port=1,mpls\_label=35 actions=push\_mpls:0x8847,set\_fied:30->mpls\_label,set\_mpls\_ttl(128),output:2
}
\end{center}
...
En pocas palabras se le colocan dos cabezales mpls al paquete y se reenvía a trav\'es del puerto 2(interfaz nf0).\\

Al llegar al nodo \textit{Galois}, el siguiente salto en el camino es implementado por el siguiente flujo en la tabla de dicho nodo:

\begin{center}
\textit{cookie=0.0, duration=1846.955s, table=0, n\_packets=0, n\_bytes=0, priority=5, \\
mpls,in\_port=2,mpls\_label=30 actions=pop\_mpls:0x8847,output:4 }
\end{center}

Se le coloca una nueva etiqueta mpls externa con el valor 30, y se reenvia por el puerto n\'umero 4(interfaz nf2).\\

Finalmente en el nodo \textit{Poisson} el tramo final del camino es implementado por el siguiente flujo en su tabla:

\begin{center}
\textit{cookie=0.0, duration=1978.551s, table=0, n\_packets=0, n\_bytes=0, priority=5, \\
mpls,in\_port=2,mpls\_label=35 actions=pop\_mpls:0x0800,output:1 }
\end{center}
...
Para verificar el correcto funcionamiento de los flujos OpenFlow que implementan la clasificaci\'on de tr\'afico, se generan paquetes asociados a un servicio creado, y se observa la manipulaci\'on de estos paquetes al ingresar a la red del prototipo.
