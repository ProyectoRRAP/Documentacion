...
En este documento se ofrece una guía paso a paso para construir a partir de una PC de escritorio, una tarjeta NetFPGA-10G y diferentes herramientas de software un switch OpenFlow 1.3 IP/MPLS hibrido; lo que denominamos RAUSwitch.\\
...
En caso de querer profundizar en alguna de las etapas, recurriendo al correspondiente material tomado como referencia se incluye la referencia a dicho material en caso de existir.

% ************************** Antes de empezar ******************************

\section{Plataforma utilizada}

A continuación se especifican las herramientas de software utilizadas en la construcción del switch, con sus correspondientes versiones de producto, así como las características del hardware utilizado.
...
\item Memoria: Kingston DDR3 8GB
\end{itemize}
...
\begin{enumerate}
\item Instalación de un sistema operativo
...
La instalación de la suite Xilinx ISE SDK la realizaremos desde el código fuente por lo que debemos primero que nada descargar dichos archivos desde la página oficial de Xilinx. En este trabajo se descargo el siguiente archivo:
...
Para trabajar con las herramientas de Xilinx es necesario contar con licencias correspondientes para cada una de ellas y en particular dependiendo del tipo de hardware con el que se trabaje dependerá el tipo de licencia necesaria. En nuestro caso en particular se necesita una licencia completa(full license), y vale la pena destacar que con una licencia trial no se cuentan con las licencias apropiadas para compilar los proyectos de NetFPGA con los que se trabajara.\\
...
\caption[SW9 en posicion PCIe]{SW9 en posicion PCIe}
\label{fig:Img2}
\end{figure}
...
\caption[Posiciones DIP SW1 SW2 SW6 SW10]{Posiciones DIP SW1 SW2 SW6 SW10}
\label{fig:Img3}
\end{figure}
...
\caption[Conector ATX]{Conector ATX}
\label{fig:Img4}
\end{figure}
...
\caption[Cable samtec twinax]{Cable samtec twinax}
\label{fig:Img5}
\end{figure}

\item Conectar un par de cables 10GE SFP+ a las interfaces de la tarjeta. Cada cable conecta dos puertos adyacentes formando dos “loops”(ver figura ~\ref{fig:Img6}):
...
\caption[Cable serial puerto COM]{Cable seria puerto COM}
\label{fig:Img7}
\end{figure}
...
\caption[Tarjeta NetFPGA esquema de instalación]{Tarjeta NetFPGA esquema de instalación}
\label{fig:Img9}
\end{figure}
...
\caption[Img10]{tarjeta NetFPGA instalada en una PC}
\label{fig:Img10}
\end{figure}
...
\begin{bash}
# cd projects/production\_test/sw/
\end{bash}
...
A continuación se describen los pasos necesarios para programar las tarjetas NetFPGA en particular con el proyecto ReferenceNIC. En lo que prosigue se utiliza NetFPGA\_ROOT para referirse al directorio donde se encuentra descargado el código fuente de NetFPGA.
...
\item En el directorio NetFPGA\_ROOT/projects/reference\_nic/hw/ cambiar en el archivo \textit{system.mhs} lo siguiente:
...
\item En el directorio NetFPGA\_ROOT/projects/reference\_nic/hw/nf10 cambiar en el archivo \textit{xflow.opt} lo siguiente: 
...
\begin{enumerate}
\item Posicionarse en el directorio NetFPGA\_ROOT/projects/reference\_nic/sw/host/driver
...
# cd NetFPGA_ROOT
# cd tools/scripts
# ./impact_run ../../projects/reference_nic/bit/reference_nic.bit 
...
\begin{bash}
# cd ../sw/host/pcieprog
# ./nf10\_configure -b ../../../bitfiles/reference_nic.bin -f a
\end{bash}
...
Para no cargar el driver del reference\_nic cada vez que se reinicie la máquina podemos modificar el archivo /etc/rc.local agregando la siguiente línea:
...
Posteriormente copiar los archivos de extensión .o y .ko del directorio OVS\_SOURCE\_PATH/datapath/linux al directorio anteriormente creado.\\
...
El software de ruteo Quagga puede instalarse un gestpt de aplicaciones(apt-get) o desde su codigo fuente; en este tutorial utilizamos esta ultima opción. 
